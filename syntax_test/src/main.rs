...
impl SyntaxHighlightTester {
    ...
    fn load_language(&self, name: &str) -> Option<Language> {
        // Try different naming conventions for grammar libraries
        let possible_names = vec![
            format!("{}.so", name),
            format!("tree-sitter-{}.so", name),
            format!("libtree-sitter-{}.so", name),
        ];

        for lib_name in possible_names {
            let lib_path = self.grammars_dir.join(&lib_name);
            if !lib_path.exists() {
                continue;
            }

            unsafe {
                if let Ok(library) = libloading::Library::new(lib_path) {
                    // Try different function names
                    let possible_functions = vec![
                        format!("tree_sitter_{}", name),
                        format!("tree_sitter_{}_wasm", name),
                    ];

                    for func_name in possible_functions {
                        if let Ok(language_fn) = library.get::<unsafe extern "C" fn() -> Language>(func_name.as_bytes()) {
                            return Some(language_fn());
                        }
                    }
                }
            }
