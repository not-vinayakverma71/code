...
fn start_stream_session(
    ...
    create_effect(move |_| {
        ...
        let has_more = {
            let mut q = queue_effect.lock().unwrap();
            let event_opt = q.pop_front();
            let has_more = !q.is_empty();
            drop(q);
    ...
    std::thread::spawn(move || {
        println!("[SSE] Thread started, prompt length: {}", prompt_clone.len());
        match stream_gemini_sse(&prompt_clone, move |ev| {
            queue_publish.lock().unwrap().push_back(ev);
            register_ext_trigger(trigger_bg);
        }) {
            Ok(_) => println!("[SSE] Stream completed successfully"),
...
fn app_view() -> impl IntoView {
    ...
    // Auto-run UI streaming test ONLY if GEMINI_UI_TEST is explicitly set
    if env::var("GEMINI_UI_TEST").ok().is_some() {
...
fn stream_gemini_sse(
    ...
    println!("[SSE] API key loaded: {} chars", api_key.len());
    let base_url = format!(
        "https://generativelanguage.googleapis.com/v1beta/models/{}:streamGenerateContent?alt=sse",
        gemini_model()
    );
    println!("[SSE] Connecting to: {}", base_url);

    // Build robust client: enable keep-alives but set read timeout to avoid indefinite hang
    let client = reqwest::blocking::Client::builder()
        .connect_timeout(Duration::from_secs(10))
        .timeout(Duration::from_secs(120)) // Overall timeout for slow responses
        .pool_idle_timeout(Duration::from_secs(30))
        .tcp_keepalive(Duration::from_secs(30))
        .user_agent("Lapce-Gemini-Streaming/0.1")
        .build()
        .map_err(|e| format!("HTTP client build failed: {}", e))?;
    println!("[SSE] HTTP client built");
    let payload = json!({
        "contents": [{
            "parts": [{ "text": prompt }]
        }],
    ...
    let mut response = loop {
        ...
        let send_res = client
            ...
            .json(&payload)
            .send();
        println!("[SSE] Request completed, processing response...");

        match send_res {
            Ok(resp) => {
                println!("[SSE] Got response status: {}", resp.status());
    ...
    // Use BufReader to properly handle UTF-8 encoding and line boundaries
    ...
    loop {
        ...
        match reader.read_line(&mut line) {
            Ok(0) => break, // EOF
            Ok(_) => {
                let line = line.trim_end(); // Remove trailing \n and \r
                if !line.is_empty() {
                    // Debug: print raw SSE line with timestamp
                    let ts = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_millis();
                    println!("[SSE @{}ms] {}", ts, line);
                    ...
                    if let Some(json_str) = line.strip_prefix("data: ") {
                        ...
                        if trimmed == "[DONE]" {
                            ...
                            eprintln!("[SSE] Failed to parse JSON: {}", trimmed);
                        }
                    }
                }
            }
