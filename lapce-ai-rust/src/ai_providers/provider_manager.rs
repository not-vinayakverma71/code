...
impl ProviderManager {
    ...
    async fn create_provider(config: &ProviderConfig) -> Result<Box<dyn AiProvider + Send + Sync + 'static>> {
        // This will be implemented when we port the actual providers
        bail!("Provider creation not yet implemented for: {}", config.name)
    }
