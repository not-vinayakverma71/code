...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    results_clone.memory_baseline_mb.store(results_mut.memory_baseline_mb.to_bits(), Ordering::Relaxed);
    results_clone.memory_peak_mb.store(results_mut.memory_peak_mb.to_bits(), Ordering::Relaxed);
    results_clone.memory_overhead_mb.store(results_mut.memory_overhead_mb.to_bits(), Ordering::Relaxed);
    ...
    // Print final report
    println!();
    results_mut.memory_overhead_mb = results.memory_overhead_mb.load(Ordering::Relaxed) as f64;
