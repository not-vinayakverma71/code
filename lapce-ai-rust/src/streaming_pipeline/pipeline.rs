            ...
            while let Some(result) = stream.next().await {
                match result {
                    Ok(chunk) => {
                        ...
                        let permit = {
                            let pipeline = self.lock().await;
                            match pipeline.backpressure.acquire().await {
                                Ok(p) => p,
                                Err(e) => {
                        ...
                        let tokens = {
                            let mut pipeline = self.lock().await;
                        ...
                        {
                            let pipeline = self.lock().await;
                            pipeline.backpressure.adapt_capacity(duration).await;
                        }
