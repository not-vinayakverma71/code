...
use crate::ai_providers::provider_pool::{ProviderPool, ProviderPoolConfig, ProviderResponse};
...
impl IpcServer {
    pub async fn new(socket_path: &str) -> Result<Self, IpcError> {
        ...
        let circuit_breaker = Arc::new(CircuitBreaker::with_config(circuit_config));
    ...
    async fn handle_connection(&self, mut stream: SharedMemoryStream) -> Result<(), IpcError> {
        ...
        loop {
            ...
            let response = match self.process_message(&buffer[..msg_len], &mut conn_metrics).await {
                Ok(resp) => resp,
                Err(e) => {
