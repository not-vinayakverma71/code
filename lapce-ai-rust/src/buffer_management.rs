...
pub use crate::stream_transform::{
    ApiStream, 
    ApiStreamChunk, 
    ApiStreamTextChunk, 
    ApiStreamUsageChunk, 
...
mod tests {
    ...
    fn test_stream_chunk_creation() {
        let text_chunk = ApiStreamChunk::text("Hello".to_string());
        assert!(text_chunk.is_text());
        assert_eq!(text_chunk.as_text(), Some("Hello"));
        
        let usage_chunk = ApiStreamChunk::usage(100, 50);
        assert!(usage_chunk.is_usage());
        
        let error_chunk = ApiStreamChunk::error("ERROR".to_string(), "Test error".to_string());
    ...
    fn test_stream_buffer() {
        ...
        buffer.push(ApiStreamChunk::usage(10, 5));
