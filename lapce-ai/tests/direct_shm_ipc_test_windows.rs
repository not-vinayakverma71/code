/// Direct Shared Memory IPC Test - Windows Version
/// Tests IPC with manually created buffers using Windows CreateFileMapping API
...
const TEST_BASE: &str = "windows_direct_shm_test";

#[cfg(windows)]
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn test_windows_direct_shm_ipc() {
    println!("\n╔═══════════════════════════════════════════════════════════╗");
    println!("║ DIRECT SHARED MEMORY IPC TEST - Windows                  ║");
    println!("║ Validates: Win32 API, atomics, message passing           ║");
    println!("╚═══════════════════════════════════════════════════════════╝\n");
    ...
    println!("\n✅ All Windows direct SHM tests passed!");
}
...
async fn test_buffer_create_open() {
    ...
    // Create buffer using Win32 CreateFileMapping
    let buffer1 = WindowsSharedMemory::create(&buffer_name, 1024 * 1024)
        .expect("Failed to create buffer");
    
    // Open same buffer (simulates second process)
    let _buffer2 = WindowsSharedMemory::create(&buffer_name, 1024 * 1024)
...
async fn test_message_passing() {
    ...
    println!("✅ Windows message passing validated");
}
